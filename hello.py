...
def fingerprint_custom():
    ...
    with sentry_sdk.push_scope() as scope:
        ...
        try:
            ...
            return jsonify({"error": str(e), "group": group, "fingerprint": ["custom-group", group], "sentry_event_id": str(event_id)})
...
def fingerprint_transaction():
    ...
    with sentry_sdk.push_scope() as scope:
        ...
        try:
            ...
            event_id = sentry_sdk.capture_exception(e)
            return jsonify({"error": str(e), "transaction_type": transaction_type, "fingerprint": ["transaction-error", transaction_type], "sentry_event_id": str(event_id)})
