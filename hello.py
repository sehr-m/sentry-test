...
def message_capture():
    ...
    try:
        ...
        logger.warning(f"Failed to capture message with Sentry: {e}")
        return jsonify({"status": "error", "level": level, "message": message, "error": str(e)}), 500
...
def message_event():
    ...
    try:
        ...
        logger.warning(f"Failed to capture event with Sentry: {e}")
